	...
	function serve(message, signal, capabilities) {
		...
		const teardown = (graceful) => {
			...
			if (graceful) writeFrame(`event: message\ndata: ${JSON.stringify({
				...
				result: {
					resultType: "complete",
					_meta: { [_modelcontextprotocol_core_internal.SUBSCRIPTION_ID_META_KEY]: subscriptionId }
				}
			})}\n\n`);
			closed = true;
			unsubscribe?.();
	...
	*/
	teardownAll() {
		...
		for (const id of this._subs.keys()) out.push({
			...
			result: {
				resultType: "complete",
				_meta: { [_modelcontextprotocol_core_internal.SUBSCRIPTION_ID_META_KEY]: id }
			}
		});
		this._subs.clear();
		return out;
...
//# sourceMappingURL=mcp-CzU40gwu.cjs.map